{{-- Langkah 0 — Jenis Organisasi & Titik Mula (§6 L0) --}}
<div>
    <label class="block text-sm font-semibold text-ink">Jenis organisasi <span class="text-red-600">*</span></label>

    {{-- Kumpulan: Masjid & Surau --}}
    <p class="mt-3 text-xs font-semibold tracking-wide text-ink/50 uppercase">Masjid &amp; Surau</p>
    ...
    {{-- Kumpulan: Pertubuhan / NGO --}}
    <p class="mt-6 text-xs font-semibold tracking-wide text-ink/50 uppercase">Pertubuhan / NGO</p>
    <div class="mt-2 grid gap-3 sm:grid-cols-2">
        @foreach ([
            ['ngo_komuniti', 'Pertubuhan / NGO (Komuniti)', 'Laman padat badan kebajikan & komuniti — program, derma, sukarelawan', '8–10 halaman', 'HandHeart'],
            ['ngo_penuh', 'Pertubuhan / NGO (Penuh)', 'Laman penuh yayasan/pertubuhan — organisasi, keahlian, berita, muat turun', '14+ halaman', 'Building'],
        ] as [$value, $title, $desc, $pages, $icon])
            <label class="relative cursor-pointer">
                <input type="radio" wire:model.live="data.tier" value="{{ $value }}" class="peer sr-only">
                <div class="h-full rounded-2xl border-2 border-sand bg-white p-4 transition peer-checked:border-brand-600 peer-checked:bg-brand-50 peer-checked:ring-1 peer-checked:ring-brand-600/20 hover:border-brand-600/40">
    ...
    {{-- Pematuhan .gov.my — masjid/surau sahaja (disembunyi untuk NGO) --}}
    @if (! in_array($data['tier'] ?? '', ['ngo_komuniti', 'ngo_penuh'], true))
        <div class="mt-8 rounded-xl border border-sand bg-cream p-4">
            <label class="flex cursor-pointer items-start gap-3">
                ...
                <span>
                    ...
                    <span class="mt-1 block text-xs text-ink/60">
