...
<div>
    <div class="mb-4 flex items-center justify-between rounded-xl bg-brand-50 px-4 py-3">
        <p class="text-sm text-ink/70">Tanda halaman yang anda mahu pada laman. Halaman utama &amp; hubungi kekal wajib.</p>
        <span class="shrink-0 rounded-full bg-brand-600 px-3 py-1 text-sm font-semibold text-white">Anggaran: {{ $enabledCount }} halaman</span>
    </div>

    {{-- Halaman utama (wajib) --}}
    <label class="mb-4 flex items-center gap-3 rounded-xl border border-brand-600/30 bg-white px-3 py-2.5">
        <input type="checkbox" checked disabled class="h-4 w-4 accent-brand-600">
        ...
        <div class="mb-4">
            ...
            <div class="grid gap-2 sm:grid-cols-2">
                @foreach ($keys as $key)
                    ...
                    <label class="flex items-start gap-2.5 rounded-xl border border-sand bg-white px-3 py-2.5 cursor-pointer hover:border-brand-600/40">
                        <input type="checkbox"
    ...
    <div class="mt-6 rounded-xl border border-sand bg-cream p-4">
        <div class="flex items-center justify-between">
            <h3 class="text-sm font-semibold">Halaman tambahan (custom) — max 3</h3>
            @if (count($data['custom'] ?? []) < 3)
                <button type="button" wire:click="addRow('custom')" class="text-sm font-medium text-brand-600 hover:underline">+ Tambah</button>
            @endif
        </div>
        @foreach ($data['custom'] ?? [] as $i => $custom)
            <div class="mt-3 grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                <input type="text" wire:model.blur="data.custom.{{ $i }}.name" placeholder="Nama halaman" class="rounded-xl border border-sand px-3 py-2 text-sm">
                <input type="text" wire:model.blur="data.custom.{{ $i }}.purpose" placeholder="Tujuan" class="rounded-xl border border-sand px-3 py-2 text-sm">
