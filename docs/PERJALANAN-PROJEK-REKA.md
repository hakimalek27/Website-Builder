...
### Pembersihan galeri — buang "browse", tambah thumbnail gov (12 Jul — `4a0291b`)
Owner semak semula galeri: **"gambar kosong"**. Audit Claude-in-Chrome (`document.querySelectorAll('[wire:key^=tpl-]')` → per-kad `img.complete && naturalWidth>0`) mendedah **22/22 thumbnail sebenar muat sempurna** — yang "kosong" hanya **6 entri pautan "browse"** ThemeForest (`/category/wordpress?term=...`, tag style `browse`) yang **bukan templat rujukan** (dipapar placeholder huruf). **Tindakan:** (1) buang 6 browse (JSON filter `!in_array('browse',style_tags)` + DB `where('url','like','%/category/%')->delete()` — real theme guna `/item/`); (2) 2 laman masjid gov (Masjid Wilayah + Masjid Negara) yang tiada thumbnail → **screenshot laman sebenar** guna **Chrome headless** (`--headless=old --window-size=1280,720 --virtual-time-budget=10000 --screenshot`) → GD resize 800×450 q82. Hasil: **40 tema bersih (18 masjid + 22 NGO), SEMUA thumbnail wujud, 0 placeholder**. Klik disahkan langsung: Butiran (modal), pautan "Lihat demo penuh ↗" (`href` betul + `target=_blank rel=noopener noreferrer`), Pilih (banner "Templat dipilih" + snapshot ke relasi `sections` step_2). **Gotcha baharu:** guard sandbox PowerShell sekat skrip yang ada `Remove-Item` + laluan literal `C:\Program` (chrome.exe) serentak — buang `Remove-Item` (`--screenshot` menimpa sendiri). `TemplateCatalogTest` guna `toBeGreaterThanOrEqual(14)` → 40 kekal lulus (32/32 Phase16 hijau).

**Isu kedua — thumbnail Katalog Templat admin pecah (`17191c4`).** Semasa "semak semua", audit admin (`/admin/template-catalogs`, sesi kongsi profil Chrome) dedah **10/10 thumbnail admin pecah** (`http://localhost/storage/...` — Filament `ImageColumn->disk('public')` jana URL **mutlak APP_URL**, tak padan serve host :8237). Fix: `getStateUsing(fn ($r) => request()->getSchemeAndHttpHost().'/storage/'.$r->thumbnail_path)` (Filament pulangkan URL sah terus jika lulus `filter_var(FILTER_VALIDATE_URL)` — path relatif `/storage` GAGAL cek itu, jadi mesti URL penuh dari host permintaan). Disahkan: admin 10/10 muat. **Pelajaran:** wizard (blade) & admin (Filament) = 2 permukaan berbeza jana URL imej berlainan — kedua perlu ikut host permintaan, bukan APP_URL. Suite penuh **349 hijau**.

### Pautan demo LANGSUNG setiap tema (`9ab5662`)
Owner: pautan demo patut terus ke demo hidup, bukan halaman jualan ThemeForest. **Kaedah kutip (Claude-in-Chrome):** butang "Live Preview" TF = `themeforest.net/item/{slug}/full_screen_preview/{id}` (deterministik dari URL item) → redirect ke `preview.themeforest.net` (Cloudflare ~5s, lepas sendiri utk browser sebenar) → halaman preview iframe **demo pengarang sebenar**. Dari tab yg SUDAH pada `preview.themeforest.net` (Cloudflare lepas), `fetch` same-origin 37 halaman preview (ringan ~9KB) → ekstrak `iframe src`. **Hasil:** 31 URL demo pengarang langsung (cth `naudummy.com/muezzin`, `wp2.yogsthemes.com/newwp/alquran`); 4 gagal fetch kerana demo pengarang **http://** (mixed-content dari tab https) → guna `full_screen_preview` (TF serve via https; the-charity=`zozothemes.com` disahkan render); 2 demo pengarang **MATI** (incharity `charity.sdemo.site` + womensfoundation `envato.kroxiedigital.com` — "site can't be reached") → **kedua-dua tema DIBUANG** (owner arah) → katalog **38 tema (18 masjid + 20 NGO)**. **Gotcha:** (1) no-cors `fetch` liveness TAK boleh dipercayai — false-positive `naudummy.com` (hidup, sekat fetch) & true-positive `charity.sdemo.site` (mati); yang **tidak** throw = pasti dapat respons HTTP = hidup. (2) `full_screen_preview` iframe demo pengarang LIVE — jika pengarang mati, preview pun mati (jadi item-page fallback utk 2 mati). (3) filter jangan tolak domain hanya kerana ada substring "envato" (`envato.kroxiedigital.com` = domain pengarang). `TemplateCatalogSeeder` +baca `demo_url`; blade banner guna `demo_url`. **Nota kekal:** URL demo pihak ketiga — semak berkala.
